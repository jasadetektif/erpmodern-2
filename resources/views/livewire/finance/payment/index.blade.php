    ...
    <div class="py-4">
        <div class="bg-white p-6 rounded-lg shadow-lg">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-semibold">{{ __('Payment List') }}</h2>
                @can('manage finance')
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Processed By') }}</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($payments as $payment)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap font-medium text-emerald-600">{{ $payment->payment_number }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $payment->invoice->invoice_number }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">{{ $payment->processedBy->name }}</td>
                            </tr>
                        @empty
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            ...
                            <div class="mt-4 space-y-4">
                                <div>
                                    ...
                                    <select wire:model.live="invoice_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                        <option value="">{{ __('Select Invoice') }}</option>
                                        @foreach($unpaidInvoices as $invoice)
                                            <option value="{{ $invoice->id }}">{{ $invoice->invoice_number }} - {{ $invoice->supplier->name }} (Rp {{ number_format($invoice->total_amount, 0, ',', '.') }})</option>
                                        @endforeach
                                    </select>
